...
You will get a table with 1309 rows and 13 columns. Most of the column names are self explanatory. Where would you start in familiarising yourself with this dataset? The best way is often with simple plots.

####How are ages distributed?

Choose histogram in the plot options and Apply. Then click on the age column (clicking the header chooses all rows unless you have drag selected some rows). Then press the plot button on the toolbar. You will see this histogram:

<div style="width: 500px;">
<img src="/img/titanic_ages.png" width="400px">
</div>

Say you want to break down the age distributions by another column representing some categories like sex. Select the age and sex columns by ctrl clicking in the header and then replot. The plot should look like this:
...
It would be better to have the plots grouped by category. To do this we reset the row index and convert them to normal columns. Do this by right=licking onthe row header and choosing 'reset index'. You can also flatten the column index for embarked count so it just has one name. Now it looks like a normal table where you can select multiple columns as above and choose grouping in the plot options. If you select sex,count and group by sex you will get the following:

<div style="width: 500px;">
<img src="/img/agg_result_plot2.png" width="400px">
</div>

Better, but we don't know who survived. To add this we set the survived column as the row index. We can also include the pclass data and use this as the secondary groupby. When we replot we get this result:

<div style="width: 500px;">
<img src="/img/agg_result_plot3.png" width="500px">
</div>

This is more informative. As you can see the index is important for labelling of the x-axis. The same table can be manipulated in other ways to get different representations.

In Pandas such operations are referred to as 'split-apply-combine'. See [this link](http://pandas.pydata.org/pandas-docs/stable/groupby.html) to learn more.
...
This means any values between 0 and 1 -> low and between 1 and 3 -> high. This creates a new column called 'pclass_binned' (you can rename it). You should perform this on the original table so the grouping can be redone and then repeat the above steps.

####Which families lost the most members

This is a specific kind of question that you might want to ask about some data. In this case it's accomplished quite easily but with several steps.

1. Extract the family name from the name field. Right click te header and choose 'String operation'. Then pick 'split' as the function. The separator is a comma by default. Then click ok, this produces 2 new columns called name_0 and name_1. The first is the surname. You can rename it.
